         ...
         require_once "../model/read_rv.php";
            if (count($liste_rv)>0) { 
                foreach ($liste_rv as $colonnes) {
                    ...
                    $sq="SELECT * FROM client WHERE id=".$Onrv["id_rv"]." ";
                    $quer=$db->prepare($sq);
                    $quer->execute();
                    $client= $quer->fetch(PDO::FETCH_ASSOC);
   ?>
 <!-- rediriger l'utilisateur dans  -->
   <script>
        function  resetAction(id,name){
            window.location.replace(`../view/dasboard.php?id_rv_get=${id}&name=${name}`);
        }
   </script>
                 ...
                 <div class="modal-body">
                    <div class="btn-exit"><button type="button" class="btn-close " data-bs-dismiss="modal" aria-label="Close"></button></div>
                       <div class="alert alert-danger" role="alert">
                        ...
                        <div id="cont-btn">
                          <button type="button" onclick="resetAction(<?=$Onrv['id_rv']?>, '<?=$client['nom_entreprise_ou_user']?>');" class="btn btn-secondary text-black mt-3 me-1" >Non</button>
